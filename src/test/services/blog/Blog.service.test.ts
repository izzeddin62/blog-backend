...
import { blogs, blogDtos } from '../../fixtures/blogs';
import { BlogPersistanceMock, mockBlogPersistance } from '../../mocks/blog.persistance.mock';
...
describe('Blog service', () => {
  ...
  describe('GET BLOG', () => {
    it('should return a blog when getOneById is called with an valid id', async () => {
      const blog = blogs[0];
      mockedBlogPersistance.getOneById.mockResolvedValue(blog);
    ...
    it('should return null when getOneById is called with an invalid id', async () => {
      mockedBlogPersistance.getOneById.mockResolvedValue(null);
  ...
  describe('GET ALL BLOGS', () => {
    it('should return all blogs when getAll is called', async () => {
      mockedBlogPersistance.getAll.mockResolvedValue(blogs);
      const returnedBlogs = await blogService.getAll();
      expect(returnedBlogs).toEqual(blogs);
      expect(mockedBlogPersistance.getAll).toHaveBeenCalled();
    });
  });
  ...
  describe('GET MY BLOGS', () => {
    it('should return all blogs when getAllMyBlogs is called with an ownerId', async () => {
      ...
      mockedBlogPersistance.getMyBlogs.mockResolvedValue(myBlogs);
  ...
  describe('CREATE BLOG', () => {
    it('should create a blog when create is called with valid data', async () => {
      ...
      mockedBlogPersistance.create.mockResolvedValue(blog);
  ...
  describe('UPDATE BLOG', () => {
    it('should update a blog when update is called with valid data', async () => {
      const blog = blogs[0];
      const blogDTO = blogDtos[0];
      mockedBlogPersistance.update.mockResolvedValue(blog);
  ...
  describe('DELETE BLOG', () => {
    it('should delete a blog when delete is called with valid data', async () => {
      const blog = blogs[0];
      mockedBlogPersistance.delete.mockResolvedValue(blog);
